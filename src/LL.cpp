...
void LL::checkPress(sf::Vector2f mousePos){
    ...
    if (DataTypes::buttonState != -1){
        ...
        switch (buttonState){
            ...
            case 2:
                for (int i = 0; i < 3; ++i){
                    if (res->minButton[i]->checkPress(mousePos))inputBox = res->minButton[i]->inputButton;
                }
                break;
            case 3:
                ...
                if (res->minButton[2]->checkPress(mousePos))inputBox = res->minButton[2]->inputButton;
                break;
        }
    }
}
